...
COMMAND="sudo /usr/bin/shellinaboxd --debug --no-beep --disable-peer-check -u ros -g ros -c /var/lib/shellinabox -p ${SIAB_PORT} --user-css ${SIAB_USERCSS}"
...
if [ "$@" = "shellinabox" ]; then
    ...
    source /home/ros/.bashrc && cd /home/ros/theia/examples/browser && node yarn run start --hostname 0.0.0.0 &
    exec ${COMMAND}
else
